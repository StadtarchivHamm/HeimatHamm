...
namespace Lumpn.Matomo
{
	public sealed class MatomoSession
	{
		...
		public static MatomoSession Create(string matomoUrl, string websiteUrl, int websiteId, byte[] userHash)
		{
			...
			// TrimEnd so a configured URL with a trailing slash can't produce "//matomo.php".
			var sb = new StringBuilder(matomoUrl.TrimEnd('/'));
			...
			sb.Append("&cid=");
			// Matomo requires cid to be exactly 16 hex chars (8 bytes of the visitor hash).
			var visitorId = new byte[8];
			Array.Copy(userHash, 0, visitorId, 0, 8);
			HexUtils.AppendHex(sb, visitorId);
			// sb.Append("&url=");
