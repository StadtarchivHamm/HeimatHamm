...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Update()
        {
            if (NumberOfPanels == 0 || isSetuping) return;
        ...
        {
            if (NumberOfPanels != Panels.Length)
            {
                Setup();
                return;
            }
        ...
        public void GoToPanel(int panelNumber)
        {
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                Panels[i].gameObject.SetActive(i == panelNumber || i == panelNumber - 1 || i == panelNumber + 1);
            }
        }
